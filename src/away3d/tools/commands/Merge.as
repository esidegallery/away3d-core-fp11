	...
	public class Merge
	{
		...
		private function collect(mesh:Mesh, dispose:Boolean):void
		{
			if (mesh.geometry) {
				...
				var UOffset:Number, UScale:Number;
				var VOffset:Number, VScale:Number;
				var i:uint;
				var len:uint;
				...
				for (subIdx = 0; subIdx < subGeometries.length; subIdx++) {
					...
					if ( mesh.subMeshes.length > subIdx ) {
						UOffset = ( mesh.subMeshes[subIdx] as SubMesh ).offsetU;
						VOffset = ( mesh.subMeshes[subIdx] as SubMesh ).offsetV;

						UScale = ( mesh.subMeshes[subIdx] as SubMesh ).scaleU;
						VScale = ( mesh.subMeshes[subIdx] as SubMesh ).scaleV;
					}
					else {
						UOffset = VOffset = 0;
						UScale = VScale = 1;
					...
					//trace ( UOffset, VOffset, UScale, VScale );
					//trace ( "--" );
					...
					for (i = 0; i < len; i++) {
						...
						vo.uvs[uIdx++] = ud[calc] * UScale + UOffset;
						vo.uvs[uIdx++] = ud[calc + 1] * VScale + VOffset;
						//vo.uvs[uIdx++] = ud[calc] + UOffset;
						//vo.uvs[uIdx++] = ud[calc + 1] + VOffset;
					}
