package away3d.containers
{
	...
	public class View3D extends Sprite
	{
		...
		public function View3D(scene:Scene3D = null, camera:Camera3D = null, renderer:RendererBase = null, forceSoftware:Boolean = false, profile:String = "baseline", mouseEnabled:Boolean = true)
		{
			super();
			_profile = profile;
		...
		override public function set width(value:Number):void
		{
			...
			if(_mouseEnabled) _hitField.width = value;
			_width = value;
		...
		override public function set height(value:Number):void
		{
			...
			if(_mouseEnabled) _hitField.height = value;
			_height = value;
		...
		public function project(point3d:Vector3D):Vector3D
		{
			...
			v.x = (v.x + 1.0) * (_globalWidth/2.0 + x);
			v.y = (v.y + 1.0) * (_globalHeight/2.0 + y);
		...
		{
			return _mouseEnabled ? _mouse3DManager.mousePicker : null;
		}

		public function set mousePicker(value:IPicker):void
		{
			if(_mouseEnabled) _mouse3DManager.mousePicker = value;
		}

		public function get touchPicker():IPicker
		{
			return _mouseEnabled ? _touch3DManager.touchPicker : null;
		}

		public function set touchPicker(value:IPicker):void
		{
			if(_mouseEnabled) _touch3DManager.touchPicker = value;
		}
		...
		private function onAddedToStage(event:Event):void
		{
			...
			if (_mouseEnabled && _shareContext)
